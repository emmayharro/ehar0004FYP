...
gap = 2;            % clear gap between bands [GHz] can increase to 2GHz if want
...
% Impairments
dc_off = 0;       % DC Offset - residual carrier leakage due to limited extinction ratio.
f_off = 0;      % Offset frequency [GHz]
...
% Lorentzian v1
% y = 0.5; %HWHM //try different filter widths
% x0 = 0; %centre
% F = linspace(0,100,101);
% I = 10; %height - does this matter??
% A = I*(y^2./((F-x0).^2+y^2));
...
for M = 4%[4,16,64] % M-QAM Level
    ...
    %for tone_fact = [0.01, 0.1, 0.2, 1/sqrt(10), 0.4, 0.5, 0.7, 1, 2, sqrt(10)]
        ...
        linewidthArray = [1e3, 1e4, 1e5, 2e5, 5e5, 8e5, 1e6, 2e6, 5e6, 8e6, 1e7, 2e7, 5e7, 8e7, 1e8]; %[Hz]
        yArray = linspace(0.1, 1, length(linewidthArray)); %HWHM
        for i = 1:15
            %for linewidth = 1e3%[1e3, 1e4, 1e5, 1e6, 1e7, 1e8]
...
%                     [Ex,Ey] = impair_OSNR(Ex,Ey,f_samp,OSNR);           %load in optical noise
                    ...
                    I = 1; %height - does this matter?? adds gain if =/=1
                    A = I*(y^2./((F-x0).^2+y^2)); %
                    %plot(F,10*log10(A))
