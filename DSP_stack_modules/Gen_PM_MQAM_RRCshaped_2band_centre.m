function Gen_PM_MQAM_RRCshaped_2band_centre(M,Baud,DACrate,Beta,Att_stop,gap,sep_fact,dc_fact,tone_fact,PE_on)
...
if mod(Nsc,2)==1
    error('Need to have an even number of sub carriers')
end
...
synch = qammod(temp,2)*exp(1i*pi/4)*sqrt(2/3*(M-1));
...
df=Baud/Nsc;
dfs=[(-df*sep_fact/2)-(gap/2) (df*sep_fact/2)+(gap/2)];   
...
TxX=sum(TxX,2);
TxY=sum(TxY,2);
% TxX=TxX(:,1); %for single sideband ...
% TxY=TxY(:,1);

TxX=TxX./sqrt(mean(abs(TxX).^2));
TxY=TxY./sqrt(mean(abs(TxY).^2));

TxX=TxX+tone_fact.*exp(1i*pi/4);
TxY=TxY+tone_fact.*exp(1i*pi/4);
...
save(namefile,'TxX','TxY','dataX','dataY','synch','rrc_filter','TxX_bits','TxY_bits','Beta','Att_stop','Baud','M','Nsc','sep_fact','gap','Nsc','dfs');
