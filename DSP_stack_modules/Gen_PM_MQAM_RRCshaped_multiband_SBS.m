function Gen_PM_MQAM_RRCshaped_multiband_SBS(M,Baud,DACrate,Beta,Att_stop,Nsc,sep_fact,dc_fact,tone_fact,linewidth,BFS,PE_on)
...
tonef=(dfs(end-1)+dfs(end))/2;

TxX=TxX+tone_fact.*exp(-1i*2*pi*tonef*1e9.*t);%+0.3.*exp(-1i*2*pi*(tonef+BFS)*1e9.*t);
TxY=TxY+tone_fact.*exp(-1i*2*pi*tonef*1e9.*t);
...
save(namefile,'TxX','TxY','dataX','dataY','synch','rrc_filter','TxX_bits','TxY_bits','Beta','Att_stop','Baud','M','Nsc','sep_fact','tonef','BFS','dfs');
