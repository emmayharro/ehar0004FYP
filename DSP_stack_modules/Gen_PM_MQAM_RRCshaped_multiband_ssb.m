function Gen_PM_MQAM_RRCshaped_multiband_SSB(M,Baud,DACrate)
...
fid = fopen([sav_dir '/' 'ssb_real_X.txt'], 'wt');
fprintf(fid, '%2.8f\n',real(TxX));
fclose(fid);
fid = fopen([sav_dir '/' 'ssb_imag_X.txt'], 'wt');
fprintf(fid, '%2.8f\n',imag(TxX));
fclose(fid);
fid = fopen([sav_dir '/' 'ssb_real_y.txt'], 'wt');
fprintf(fid, '%2.8f\n',real(TxY));
fclose(fid);
fid = fopen([sav_dir '/' 'ssb_imag_y.txt'], 'wt');
fprintf(fid, '%2.8f\n',imag(TxY));
fclose(fid);
namefile = [sav_dir '/' 'ssb_transmit_data.mat'];
save(namefile,'TxX','TxY','dataX','dataY','synch','rrc_filter','TxX_bits','TxY_bits','Beta','Att_stop','Baud','M','Nsc','sep_fact','Nsc','dfs');
